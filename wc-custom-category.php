...
class WC_Custom_Category_list_shortcode {
	...
	public function register_shortcodes() {
		...
		add_shortcode( 'wc_cat_grid', function( $atts ) { return $this->render_categories_grid( $atts ); } );
	}
	...
	private function render_categories_grid( $atts ) : string {
		if ( ! $this->wc_active() ) { return ''; }
		...
		$hideEmpty = ( 'no' === strtolower( (string) $atts['hide_empty'] ) ) ? false : true;
		...
		if ( is_wp_error( $terms ) || empty( $terms ) ) {
			return '<div class="wcccs-empty">' . esc_html__( 'No categories found.', 'wcccs' ) . '</div>';
		}
		...
		foreach ( $terms as $term ) {
			...
			$img      = $thumb_id ? wp_get_attachment_image( $thumb_id, 'woocommerce_thumbnail' ) : '<div class="wcccs-cat-placeholder"></div>';
			$link     = get_term_link( $term );
			if ( is_wp_error( $link ) ) { $link = '#'; }
			?>
			<div class="wcccs-cat-item">
				<a class="wcccs-thumb" href="<?php echo esc_url( $link ); ?>"><?php echo $img; ?></a>
				<h3 class="wcccs-title"><a href="<?php echo esc_url( $link ); ?>"><?php echo esc_html( $term->name ); ?></a></h3>
<!-- 				<div class="wcccs-count"><?php echo isset( $term->count ) ? esc_html( $term->count ) : ''; ?></div> -->
			</div>
			<?php
		}
		echo '</div>';
	...
	private function print_styles_once() {
		...
		<style>
			...
			.wcccs-note,.wcccs-empty{padding:10px;background:#f8f9fa;border:1px solid #eee;border-radius:6px}
					.wcccs-cat-item{border:1px solid #eee;padding:12px;border-radius:8px;background:#fff;text-align:center}
			.wcccs-cat-placeholder{width:100%;aspect-ratio:1/1;background:#f3f4f6;border-radius:6px}
		</style>
		<?php
	}
}
