...
class WC_Product_Personalizer {
    ...
    public function __construct() {
		...
		// Frontend hooks - Enhanced for better theme compatibility
		add_action( 'woocommerce_single_product_summary', array( $this, 'display_personalization_inputs' ), 25 ); // Left side: Inputs after price
		add_action( 'woocommerce_single_product_image', array( $this, 'display_personalization_overlay' ), 15 ); // Right side: Overlay on image
		add_action( 'woocommerce_product_thumbnails', array( $this, 'display_personalization_overlay' ), 5 ); // Fallback hook for thumbnails
		add_action( 'wp_footer', array( $this, 'inject_personalization_overlay' ) ); // Ultimate fallback: Inject via footer JS
		add_action( 'wp_footer', array( $this, 'inject_personalization_inputs' ) ); // Ultimate fallback: Inject via footer JS if hooks fail
		add_action( 'woocommerce_before_add_to_cart_form', array( $this, 'display_personalization_inputs' ), 10 ); // Early fallback
            ...
            <table class="wp-list-table widefat fixed striped">
                ...
                <tbody>
                    <?php if ( $products ) : ?>
                        ...
                        <?php endforeach; ?>
                    <?php else : ?>
                        <tr><td colspan="2"><?php esc_html_e( 'No personalized products found.', 'wc-product-personalizer' ); ?></td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
        ...
        ?>
            <div class="personalization-input-group" style="margin-bottom: 20px; padding: 10px; background: #fff; border: 1px solid #eee; border-radius: 5px;">
                ...
                <?php if ( $element['type'] === 'text' ) : ?>
                    <input type="text" name="<?php echo esc_attr( $input_name ); ?>" id="<?php echo esc_attr( $input_id ); ?>" value="<?php echo esc_attr( $default_value ); ?>" data-element-id="<?php echo esc_attr( $element['id'] ); ?>" data-type="text" style="width: 100%; padding: 10px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px; font-size: 14px;" placeholder="Enter your text here..." />
                ...
                <?php elseif ( $element['type'] === 'image' ) : ?>
                    <input type="file" name="wc_personalize_<?php echo esc_attr( $element['id'] ); ?>" id="<?php echo esc_attr( $input_id ); ?>" accept="image/*" data-element-id="<?php echo esc_attr( $element['id'] ); ?>" data-type="image" style="width: 100%; padding: 10px; box-sizing: border-box; border: 1px solid #ccc; border-radius: 4px; font-size: 14px;" />
...
public function display_personalization_overlay() {
    ...
    if ( ! has_action( 'woocommerce_single_product_image' ) ) { // Avoid duplicates
        echo '<div class="wc-personalizer-image-container" style="position: relative; display: inline-block; width: 100%;">';
    }

    // Output overlay div
    echo '<div class="personalization-overlay" style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; z-index: 10;">' . $overlay_html . '</div>';
...
public function inject_personalization_overlay() {
    ...
    jQuery(document).ready(function($) {
        ...
        if ($gallery.length) {
            $gallery.css('position', 'relative');
            $gallery.append('<div class="personalization-overlay" style="position:absolute; top:0; left:0; width:100%; height:100%; pointer-events:none; z-index:10;"><?php echo wp_json_encode( $overlay_html ); ?></div>');
    ...
    public function add_personalization_data_to_cart_item( $cart_item_data, $product_id ) {
        if ( isset( $_POST['wc_personalize'] ) && is_array( $_POST['wc_personalize'] ) ) {
            $cart_item_data['wc_personalize'] = $_POST['wc_personalize'];
